...
export default function AdminHomePage() {
  ...
  async function handleResourceReview(resourceId: number, decision: "approve" | "reject"): Promise<void> {
    if (!token) {
      return;
    }
    try {
      await requestData(`/admin/resources/${resourceId}`, token, {
        ...
        body: JSON.stringify({
          decision,
          reason: decision === "reject" ? "Manually rejected by moderator." : "Approved by moderator."
        })
      });
      setActionMessage(`Resource ${resourceId} ${decision === "approve" ? "approved" : "rejected"}`);
  ...
  async function handlePublishAnnouncement(): Promise<void> {
    ...
    try {
      await requestData("/admin/announce", token, {
        method: "POST",
        body: JSON.stringify({ content: noticeContent.trim(), publisher: noticePublisher.trim() || "admin" })
      });
      setNoticeContent("");
    ...
    <main className={styles.page}>
      <div className={styles.headerRow}>
        <div>
          ...
          <p className={styles.subtitle}>Manage users, moderation, announcements and captain growth strategy.</p>
          <p className={styles.sessionInfo}>Signed in as {profile?.username ?? "admin"}</p>
        </div>
        <div className={styles.topActions}>
          <button type="button" className={styles.refreshBtn} onClick={() => token && void refreshAll(token)}>
            Refresh
          </button>
      ...
      <section className={styles.statsGrid}>
        ...
        <article className={styles.statCard}>
          ...
          <strong className={styles.statValue}>{stats ? `${(stats.matchRate * 100).toFixed(1)}%` : "-"}</strong>
        </article>
      </section>
      ...
      <section className={styles.panel}>
        <div className={styles.panelHeader}>
          ...
          <div className={styles.filters}>
            <select value={userStatusFilter} onChange={(event) => setUserStatusFilter(event.target.value as "all" | UserStatus)}>
            ...
            <select value={userRoleFilter} onChange={(event) => setUserRoleFilter(event.target.value as "all" | UserRole)}>
                ...
                <tr key={user.userId}>
                  ...
                  <td>
                    <div className={styles.actionInline}>
                      <button type="button" onClick={() => void handleUserStatusChange(user.userId, "active")}>Active</button>
                      <button type="button" onClick={() => void handleUserStatusChange(user.userId, "frozen")}>Freeze</button>
                      <button type="button" onClick={() => void handleUserStatusChange(user.userId, "banned")}>Ban</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <section className={styles.panel}>
        <div className={styles.panelHeader}>
          ...
          <div className={styles.filters}>
            <select value={resourceStatusFilter} onChange={(event) => setResourceStatusFilter(event.target.value as "all" | ResourceStatus)}>
            ...
            <tbody>
              {resources.map((resource) => (
                <tr key={resource.resourceId}>
                  ...
                  <td>
                    <span className={`${styles.badge} ${styles[resource.status]}`}>{resource.status}</span>
                  </td>
                  <td>{formatDate(resource.createdAt)}</td>
                  <td>{formatDate(resource.verifiedAt)}</td>
                  <td>
                    <div className={styles.actionInline}>
                      <button type="button" onClick={() => void handleResourceReview(resource.resourceId, "approve")}>Approve</button>
                      <button type="button" onClick={() => void handleResourceReview(resource.resourceId, "reject")}>Reject</button>
                    </div>
                  </td>
                </tr>
              ))}
              ...
              <tbody>
                {captains.map((captain) => (
                  <tr key={captain.captainId}>
                    ...
                    <td>
                      <button type="button" onClick={() => void handleCaptainLevelSave(captain.captainId)}>
                        Save
                      </button>
                    </td>
                  </tr>
